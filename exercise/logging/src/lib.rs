...
impl Frog {
    pub fn new() -> Self {
        ...
        debug!(target: "debug","A new Frog has been created");
        Default::default()
    }
    pub fn hop(&mut self) {
        self.energy -= 1;
        // 3. Use info!() to log that a Frog hopped, and how much energy is left
        info!(target: "info","Frog hopped. Remain energy: {}", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "warn","Run out of energy. Frog'll sleep");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!(target: "sleep","Frog sleeping");
        } else {
            self.sleeping = true;
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "trace","Default was generate");
        Frog {
            energy: 5,
            sleeping: false,
        }
