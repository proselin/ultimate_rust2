...
fn play_time(dolphin: &Dolphin) -> Result<Vec<String>> {
    ...
    //
    responses.push(dolphin.say_your_name()?);   // this can be done with an intermediate variable...
    //
...
fn main() -> Result<()> {
    ...
    for dolphin in &dolphins {
        ...
        let responses = play_time(dolphin).unwrap_or_else( |_| vec![]); // Use the `?` operator to propagate errors
        if responses.len() > 0 {
            println!("{} did a FABULOUS PERFORMANCE!", dolphin.name);
            for response in responses {
                println!("  {}", response);x
